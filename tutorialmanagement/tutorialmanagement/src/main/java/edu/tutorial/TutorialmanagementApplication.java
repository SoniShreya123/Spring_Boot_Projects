...
public class TutorialmanagementApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		Tutorial t1 = ((Object) Tutorial.builder()).title("java").description("Core Java").published(false).build();
		Tutorial t2 = ((Object) Tutorial.builder()).title("java").description("Advanced Java").published(false).build();
		Tutorial t3 = ((Object) Tutorial.builder()).title("java").description("Spring Boot with Java").published(true).build();
		Tutorial t4 = Tutorial.builder().title("python").description("Core Python").published(false).build();
		Tutorial t5 = Tutorial.builder().title("python").description("Data Science with Python").published(false)
				.build();
		Tutorial t6 = Tutorial.builder().title("python").description("Machine Learning with Python").published(true)
				.build();
